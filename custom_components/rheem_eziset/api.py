...
class RheemEziSETApi:
    ...
    def set_temp(
        ...
        self.set_param(
            session = session,
            param = f"setTemp={temp}",
            response_param = "reqtemp",
            response_check = temp
        )
    ...
    def set_session_timer(
        ...
        if session_timer is None:
            ...
            raise ConditionErrorMessage(
                type="invalid_session_timer",
                message="{DOMAIN} - No session timer was set. Ignoring call to set session timer."
            )
        ...
        self.check_control_issues(
            entity = number,
            reset_attribute = "rheem_session_timer",
            session=session
            )
    ...
    def set_param(
        ...
        if result != response_check:
            # Something wrong happened. Log error and hand back control.
            LOGGER.error(
                "%s - Error when setting %s by retrieving %s. Result was: %s",
                DOMAIN,
                param,
                page,
                data_response)
